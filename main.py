...
import models
from crud import (create_user, get_users, get_login, update_user, delete_user
                ,get_post,get_posts,get_user_posts,delete_post)
from schemas import PostCreate, UserCreate, UserUpdate,PostInDB
...
async def create_upload_file(user_id:int,file: UploadFile = File(...), db: Session = Depends(get_db)):
    try:
        ...
        user1=db.query(models.User).filter(models.User.user_id==user_id).first()
        post_c = models.Post(user_post_data=contents,user_id=user1.user_id,is_active=user1.is_active)  
        db.add(post_c)
        db.commit()
        db.refresh(post_c)
        return post_c
